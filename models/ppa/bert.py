...
import models.ppa.base
import utils.tool

class BERTTool(object):
    def init(args):
        BERTTool.bert = BertModel.from_pretrained(args.bert.location)
...
class Model(models.ppa.base.Model):
    ...
    def get_info(self, batch):
        construced_data_infos = [item['construct_data_info'] for item in batch]
        # print('construced data infos',construced_data_infos)
        construced_infos = [item['constructed_info'] for item in batch]
        # print('construced infos',construced_infos)
        last_responses = [item['last_response'] for item in batch]
        # print('last_responses',last_responses)
        # exit()
        tokenized = self.tokenizer(construced_infos, last_responses, truncation='only_first', padding=True, return_tensors='pt', max_length = self.tokenizer.max_model_input_sizes['bert-base-uncased'], return_token_type_ids=True)
        tokenized = tokenized.data
        data_tokenized = self.tokenizer(construced_data_infos, truncation='only_first', padding=True,
                                   return_tensors='pt',
                                   max_length=self.tokenizer.max_model_input_sizes['bert-base-uncased'],
                                   return_token_type_ids=True)
        data_tokenized = data_tokenized.data


        return tokenized['input_ids'].to(self.device), tokenized['token_type_ids'].to(self.device), tokenized['attention_mask'].to(self.device), data_tokenized['input_ids'].to(self.device), data_tokenized['token_type_ids'].to(self.device), data_tokenized['attention_mask'].to(self.device)


    def forward(self, batch):
        token_ids, type_ids, mask_ids, data_token_ids, data_type_ids, data_mask_ids = self.get_info(batch)
        h, utt = self.bert(input_ids = token_ids, token_type_ids = type_ids, attention_mask = mask_ids, data_input_ids = data_token_ids, data_attention_mask=data_mask_ids, data_token_type_ids=data_type_ids)
