...
class DatasetTool(object):
    ...
    def load_data(data_path):
        ...
        for dialogue_components_item in raw_data:
            construct_data_info, constructed_info, last_response, consistency = DatasetTool.get_info(dialogue_components_item, domain)
            data_item = dict()
            data_item["construct_data_info"] = construct_data_info
    ...
    def get_info(dialogue_components_item, domain):
        ...
        construct_data_info, constructed_info = DatasetTool.construct_info(kb_expanded, history_sentences)

        return construct_data_info, constructed_info, last_response, consistency
    ...
    def construct_info(kb_expanded, history_sentences):
        ...
        """
        construct_data_info = ''
        construct_data_info += ' [SOK] '
        for row in kb_expanded:
            construct_data_info += ' '.join([triple[1] + " " + triple[2] for triple in row])
            construct_data_info += ' ; '
        construct_data_info += ' [EOK] '
        ...
        return construct_data_info, construct_info
